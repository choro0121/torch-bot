...
def readVocs(corpus, corpus_name):
    ...
    # for https://github.com/choro0121/torch-bot/blob/master/nucc.txt
    pairs = []
    for i in range(len(lines) - 1):
        if lines[i][0:3] == '###' or lines[i+1][0:3] == '###':
            continue
        else:
...
def loadPrepareData(corpus):
    corpus_name = corpus.split(path_split)[-1].split('.')[0]
    try:
